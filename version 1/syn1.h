...
void setName(arb Arbre, lexeme lex[][N], int i, int j, int numeroEtat)
{
    ...
    {
        Arbre->name_etat[numeroEtat].name[ii]=lex[i][j].valeur[ii];
    }
}
...
void printArbre(arb Arbre)
{
    ...
    printf("\n");
    printf("nombre de tran est %d\n",Arbre->nom_tran);
    for(int kk=0;kk<Arbre->nom_tran;kk++)
    {
        printf("from %d to %d with input '%c' ",Arbre->tran[kk].from,Arbre->tran[kk].to,Arbre->tran[kk].input);
        for(int ii=0;ii<Arbre->tran[kk].nomP;ii++)
        {
            switch(Arbre->tran[kk].ppile[ii].piler)
            {
                case 1:
                    printf("pile %d: enpiler '%c' ",ii,Arbre->tran[kk].ppile[ii].element);
                    break;
                case -1:
                    printf("pile %d: depiler '%c' ",ii,Arbre->tran[kk].ppile[ii].element);
                    break;
                case 0:
                    printf("pile %d: rien ",ii);
                    break;
            }
        }
        printf("\n");
                ...
                switch(e)
                {
                    ...
                    case 27:
                        switch(lex[i][j].type)
                        {
                            case NOMBRE:
                                e=28;
                                int kk=0;
                                arbr->tran[nom_tran-1].from=0;
                                while(lex[i][j].valeur[kk]!='\0')
                                {
                                    arbr->tran[nom_tran-1].from=10*arbr->tran[nom_tran-1].from+(lex[i][j].valeur[kk]-'0');
                                    kk++;
                                }
                                break;
                            default:
                    ...
                    case 29:
                        switch(lex[i][j].type)
                        {
                            case NOMBRE:
                                e=30;
                                arbr->tran[nom_tran-1].to=0;
                                int kk=0;
                                while(lex[i][j].valeur[kk]!='\0')
                                {
                                    arbr->tran[nom_tran-1].to=10*arbr->tran[nom_tran-1].to+(lex[i][j].valeur[kk]-'0');
                                    kk++;
                                }
                                break;
                            default:
                    ...
                    case 32:
                        switch(lex[i][j].type)
                        {
                            case NOMBRE:
                                ...
                                arbr->tran[nom_tran-1].input=lex[i][j].valeur[0];
                                e=33;
                                break;
			                case MOT:
                                if(!length1(lex,i,j)) 
                    ...
                    case 36:
                        switch(lex[i][j].type)
                        {
                            ...
                            case Par_fe:
                                if(!p.pop(1)){error(e,lex[i][j].valeur); return 0;}
                                arbr->tran[nom_tran-1].ppile[arbr->tran[nom_tran-1].nomP-1].piler=0;
                                e=47;
                                break;
                            default:
                                error(e,lex[i][j].valeur);
                    ...
                    case 37:
                        switch(lex[i][j].type)
                        {
                            case MOT:
                                ...
                                arbr->tran[nom_tran-1].ppile[arbr->tran[nom_tran-1].nomP-1].element=lex[i][j].valeur[0];
                                arbr->tran[nom_tran-1].ppile[arbr->tran[nom_tran-1].nomP-1].piler=-1;
                                e=38;
                                break;
                            case NOMBRE:
                                if(!length1(lex,i,j)) 
