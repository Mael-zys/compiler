...
void classer(lexeme lex[][N],int i, char *p)
{
    ...
    for(int l=0;l<j;l++)
    {
        int ll=strlen(lex[i][l].valeur);
        if(lex[i][l].type==MOT && lex[i][l+1].type==Com_fe && lex[i][l].valeur[ll-1]=='*')
        {
            lex[i][l].valeur[ll-1]='\0';
        }
    }
    ...
    if(j>1 && len==1 && lex[i][j-1].type!=NOMBRE && lex[i][j-1].valeur[0]!='}' &&  lex[i][j-1].valeur[0]!='*' &&  lex[i][j-1].valeur[0]!=',' && lex[i][j-1].valeur[0]!='{' && lex[i][j-1].valeur[0]!='*' && lex[i][j-1].valeur[0]!=']' &&lex[i][j-1].valeur[0]!='/')lex[i][j-1].fi--;
